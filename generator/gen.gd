...
const TreeGenerator = preload("res://generator/tree_generator.gd")
...
func _init():
	...
	#_heightmap_noise.seed = randi()
	_heightmap_noise.period = 512
	_heightmap_noise.octaves = 8
	#_heightmap_noise.persistence = 0.4
	#_heightmap_noise.lacunarity = .6
	var tree_generator = TreeGenerator.new()
	tree_generator.log_type = LOG
	tree_generator.leaves_type = LEAVES
	#tree_generator._seed = 0606060
	for i in 16:
...
#							ore = LEAVES
#
#						buffer.set_voxel(ore, x, relative_height - 1 , z, _CHANNEL)
				# Water
					#ore and stone
